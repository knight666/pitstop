...
namespace Pitstop {
		...
		: m_Device(device)
	{
		...
		m_Form.wdgAxisLeftTrigger->setTitle("Left Trigger");
		m_Form.wdgAxisLeftTrigger->setRange(
			0.0f,
			UINT8_MAX,
			32.0f,
			XINPUT_GAMEPAD_TRIGGER_THRESHOLD);

		m_Form.wdgAxisRightTrigger->setTitle("Right Trigger");
		m_Form.wdgAxisRightTrigger->setRange(
			0.0f,
			UINT8_MAX,
			32.0f,
			XINPUT_GAMEPAD_TRIGGER_THRESHOLD);
